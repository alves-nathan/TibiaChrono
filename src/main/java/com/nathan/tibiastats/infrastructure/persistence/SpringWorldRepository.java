...
interface ScrapeJpa extends JpaRepository<Scrape, Integer> {
    ...
    @Query("select s from Scrape s where s.world = :world order by s.scrapetime desc limit 1")
...
public class SpringWorldRepository implements WorldRepositoryPort {
    ...
    public Scrape saveScrape(Scrape r){return scrapes.save(r);}
    public List<Scrape> findScrapesByWorldAndRange(World w, Instant from, Instant to){return scrapes.findRange(w, from, to);}
    public Optional<Scrape> findLatestByWorld(World w){return scrapes.findLatest(w);}
}